  ...
  <div class="report-moderation">
    ...
    <div class="table-card">
      <div class="card-header">
        <div class="card-title">举报审核列表</div>
      </div>
            ...
            <tr v-for="item in filteredReports" :key="item.report_id" :class="{ loading: loading }">
              ...
              <td>
                <div class="table-actions">
                  ...
                  <button
                    class="table-action-btn history"
      ...
      <div class="pagination-container" v-if="totalPages > 1">
        ...
        <div class="pagination-buttons">
          <button class="pagination-btn" :disabled="pageIndex === 0" @click="changePage(0)">
            首页
          </button>
          <button
          ...
          <div class="detail-grid">
            ...
            <div class="detail-item">
              ...
              <div class="detail-value">
                {{ new Date(selectedReport.create_time).toLocaleString() }}
              </div>
            </div>
            <div class="detail-item">
              <label>举报人</label>
            ...
            <div
              v-if="selectedReport.evidences && selectedReport.evidences.length > 0"
              class="detail-item full-width"
            >
              <label>举报证据</label>
              <div class="evidence-grid">
                <div
                  v-for="(evidence, index) in selectedReport.evidences"
                  :key="index"
                  class="evidence-item"
                >
          ...
          <form @submit.prevent="submitModeration" class="moderation-form">
            ...
            <div class="form-actions">
              ...
              <button
                ...
                :disabled="!isModerationFormValid || moderating"
              >
                {{ moderating ? '提交中...' : '提交审核' }}
              </button>
            </div>
            ...
            >
              ...
              <div>
                审核意见:
                {{
                  moderationForm.handleNote
                    ? `已填写(${moderationForm.handleNote.length}字)`
                    : '未填写'
                }}
              </div>
              <div>是否处罚: {{ moderationForm.applyPenalty ? '是' : '否' }}</div>
              <div v-if="moderationForm.applyPenalty">
                处罚类型: {{ moderationForm.penaltyType || '未选择' }}
              </div>
...
<script setup lang="ts">
  ...
  // 选项数据 - 与后端实际状态保持一致
  const statusOptions = ['待处理', '处理中', '已处理', '已驳回', '需要补充信息', '已关闭']

  const typeOptions = ['价格欺诈', '商品描述不符', '虚假商品', '违规内容', '恶意交易', '其他']

  const handleResultOptions = ['通过', '驳回', '需要更多信息']

  const penaltyTypeOptions = ['轻度处罚', '中度处罚', '重度处罚']
  ...
  const isModerationFormValid = computed(() => {
    ...
    const hasNote =
      !!moderationForm.handleNote &&
      moderationForm.handleNote.trim().length > 0 &&
      moderationForm.handleNote.length <= 500
    ...
    try {
      ...
      if (response.success) {
        ...
        if (showHistoryDialog.value && reportToModerate.value) {
          try {
            const historyResponse = await adminApi.getReportAuditHistory(
              reportToModerate.value.report_id
            )
  ...
  const canModerateReport = (item: ReportItem) => {
    // 允许审核的状态：待处理、处理中（可以再次审核）
    const allowedStatuses = ['待处理', '处理中']
...
<style scoped>
  ...
  .timeline-marker.history-submit {
    background-color: #2196f3;
  }
  .timeline-marker.history-start {
    background-color: #ff9800;
  }
  .timeline-marker.history-approved {
    background-color: #4caf50;
  }
  .timeline-marker.history-rejected {
    background-color: #f44336;
  }
  .timeline-marker.history-suspended {
