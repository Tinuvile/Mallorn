<template>
  <v-app id="message-app">
    ...
    <!--消息详情浮窗 -->
    <v-dialog v-model="showMessageDetail" max-width="600px" persistent>
      <v-card>
    ...
    <!-- 议价消息特殊弹窗 -->
    <v-dialog v-model="showBargainDialog" max-width="700px" persistent>
      <v-card>
        <v-card-title class="d-flex justify-space-between align-center">
        ...
        <v-card-text>
          ...
          <!-- 对话历史 -->
          <div
            class="conversation-history mb-4"
            style="
              max-height: 300px;
              overflow-y: auto;
            ...
            "
          >
            <div v-if="bargainConversation.length === 0" class="text-center text-grey">
              暂无对话历史
            </div>
            <div
              v-for="(msg, index) in bargainConversation"
              :key="index"
              class="conversation-item mb-3"
            >
              <div class="conversation-header d-flex justify-space-between align-center mb-1">
                <span
                  class="font-weight-bold text-caption"
                  :class="msg.isFromCurrentUser ? 'text-primary' : 'text-secondary'"
                >
                  {{ msg.sender }}
                </span>
              ...
              <div class="conversation-content">
                <div
                  ...
                  :class="msg.isFromCurrentUser ? 'bg-blue-lighten-5 ml-auto' : 'bg-grey-lighten-4'"
          ...
          <div class="mb-4 p-3 rounded" style="background-color: #f5f5f5">
            <div class="d-flex align-center">
              <v-icon :color="getStatusColor(currentBargainMessage.bargainStatus)" class="mr-2">
                {{ getStatusIcon(currentBargainMessage.bargainStatus) }}
              </v-icon>
              <span class="font-weight-bold"
                >当前状态：{{ getStatusDisplayText(currentBargainMessage.bargainStatus) }}</span
              >
            </div>
          </div>
          ...
          <v-text-field
            ...
            label="请输入您的反报价"
            type="number"
            v-show="showCounterOfferInput"
            class="mt-3"
            :hint="`对方报价：￥${currentBargainMessage.myOffer || currentBargainMessage.newOffer || 0}`"
            persistent-hint
          ></v-text-field>
    ...
    <!-- 换物消息特殊弹窗 -->
    <v-dialog v-model="showSwapDialog" max-width="700px" persistent>
      <v-card>
        <v-card-title class="d-flex justify-space-between align-center">
          <span class="text-h5">
            {{
              currentSwapMessage.userRole === 'sender'
                ? '我的换物请求'
                : currentSwapMessage.sender + ' 的换物请求'
            }}
          </span>
        ...
        <v-card-text>
          ...
          <div v-if="currentSwapMessage.userRole === 'sender'">
            ...
            <p class="text-body-2 mb-3">
              您向对方发起了换物请求，希望用您的《{{
                currentSwapMessage.myProductName
              }}》换取对方的《{{ currentSwapMessage.otherProductName }}》
            </p>
          ...
          <div v-else>
            ...
            <div v-if="currentSwapMessage.swapStatus !== 'pending'" class="text-center">
              <v-icon
                :icon="
                  currentSwapMessage.swapStatus === 'accepted'
                    ? 'mdi-check-circle'
                    : 'mdi-alert-circle'
                "
        ...
        <v-card-actions
          class="justify-end"
          v-if="
            currentSwapMessage.userRole === 'receiver' &&
            currentSwapMessage.swapStatus === 'pending'
          "
        >
    ...
    <!-- 接受换物确认弹窗 -->
    <v-dialog v-model="showSwapAcceptConfirm" max-width="400px" persistent>
      <v-card>
    ...
    <!-- 拒绝换物确认弹窗 -->
    <v-dialog v-model="showSwapRejectConfirm" max-width="400px" persistent>
      <v-card>
...
<script setup>
  ...
  // 反报价相关状态
  const showCounterOfferInput = ref(false)
  const counterOfferPrice = ref('')
  const showCounterError = ref(false)
  // 议价对话历史
  const bargainConversation = ref([])
    ...
    return messages.value.filter(msg => {
      switch (currentCategory.value) {
        ...
        case 'reply':
          return (
            msg.type === 'reply' ||
            (msg.type === 'notification' && !['系统通知', '校园管理员'].includes(msg.sender))
          )
        case 'swap':
  ...
  // 加载议价对话历史
  const loadBargainConversation = async orderId => {
    try {
      const response = await notificationApi.getBargainConversation(orderId)
      if (response.success && response.data) {
        bargainConversation.value = response.data
      } else {
        console.error('获取对话历史失败:', response.message)
        bargainConversation.value = []
      }
    } catch (error) {
      console.error('加载对话历史时发生错误:', error)
      bargainConversation.value = []
    }
  }
  ...
  const handleMessageClick = async message => {
    ...
    if (message.type === 'bargain') {
      ...
      // 加载对话历史
      if (message.orderId) {
        await loadBargainConversation(message.orderId)
      }
  ...
  const handleCounterOffer = async () => {
    if (!showCounterOfferInput.value) {
      showCounterOfferInput.value = true
      showCounterError.value = false
      // 隐藏拒绝输入框
      showRejectInput.value = false
      showError.value = false
      ...
      try {
        ...
        // 更新消息状态
        message.bargainStatus = 'counter_offer'
        showBargainDialog.value = false
        showCounterOfferInput.value = false
